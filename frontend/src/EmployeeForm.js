      ...
      <Page.Card title="Employee Registration"></Page.Card>
        ...
        <Form onSubmit={handleSubmit}>
          <FormGroup>
            ...
            <Input type="text" name="id" value={values.id} onChange={handleChange} id="id" placeholder="Employee ID" />
          </FormGroup>
          <FormGroup>
            {touched.name && errors.name && <p className="red">{errors.name}</p>}
            <Label for="name">Name</Label>
            <Input type="text" name="name" value={values.name} onChange={handleChange} id="name" placeholder="Employee Name" />
          </FormGroup>
          <FormGroup>
            {touched.address && errors.address && <p className="red">{errors.address}</p>}
            <Label for="address">Address</Label>
            <Input type="text" name="address" value={values.address} onChange={handleChange} id="address" placeholder="Employee Address" />
          </FormGroup>
          <FormGroup>
            {touched.email && errors.email && <p className="red">{errors.email}</p>}
            <Label for="email">Email ID</Label>
            <Input type="text" name="email" value={values.email} onChange={handleChange} id="email" placeholder="Email ID" />
          </FormGroup>
          <FormGroup>
            {touched.phone_number && errors.phone_number && <p className="red">{errors.phone_number}</p>}
            <Label for="phone_number">Phone Number</Label>
            <Input type="text" name="phone_number" value={values.phone_number} onChange={handleChange} id="phone_number" placeholder="Phone Number" />
          </FormGroup>
          <FormGroup>
            ...
            <Input type="number" name="annual_package" value={values.annual_package} onChange={handleChange} id="annual_package" placeholder="Annual Package" />
          </FormGroup>
          <FormGroup>
          ...
          <FormGroup>
            ...
            <Input type="date" name="joining_date" id="joining_date" value={values.joining_date} onChange={handleChange} />
          </FormGroup>
...
const FormikApp = withFormik({
  mapPropsToValues({ id, name, address, email, phone_number, annual_package, job_role, status, location, joining_date }) {
    return {
        id: id || '', name: name || '', address: address || '',
        email: email || '', phone_number: phone_number || '',
        annual_package: annual_package || '', job_role: job_role || '',
        status: status || '', location: location || '', joining_date: joining_date || ''
    };
  },
  handleSubmit(values, { resetForm, setSubmitting }) {
    // 1. Save to Employee Service (Go)
    ...
    // 2. Save to Salary Service (Java)
    const salaryPayload = {
        id: values.id,
        name: values.name,
        salary: parseFloat(values.annual_package),
        processDate: values.joining_date,
        status: 'Active'
    };

    fetch('/salary/create/record', {
        method: 'POST',
        body: JSON.stringify(salaryPayload),
        headers: { 'Content-Type': 'application/json' }
    }).then(res => {
        if (res.ok) alert('Employee and Salary records synchronized!');
    }).catch(err => console.error('Salary sync failed:', err));
